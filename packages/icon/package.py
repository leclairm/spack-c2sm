...
class Icon(AutotoolsPackage):
    ...
    gpu_values = ('10', '11', '12', '13', '20', '21', '30', '32', '35', '37',
                  ...
                  '86')
    variant('gpu', default='none', values=('none',) + gpu_values,
            description='Enable GPU support with the specified compute '
                        'capability version')

    variant('cuda-gcc', default=False,
    ...
    variant('claw', default='none', values=('none',) + claw_values,
    ...
    variant('mixed-precision', default=False,
    ...
    for x in spack.compilers.supported_compilers():
        if x != 'gcc':
            conflicts('+cuda-gcc', '^cuda%{0}'.format(x),
    ...
    def patch(self):
        ...
        if any(self.spec.variants[x].value != 'none'
            ...
            filter_file(r'^(\s*)!\$(\s+.*)', '#ifdef _OPENMP\n\\1  \\2\n#endif',
                        *src_files, backup=False)
    ...
    def configure_args(self):
        ...
        if '+cdi-pio' in self.spec:
            config_args.extend(['--enable-cdi-pio',
                                '--with-external-cdi',
                                '--with-external-yaxt'])
        ...
        if self.compiler.name == 'gcc' or self._compiler_is_mixed_gfortran():
            ...
            config_vars['ICON_FCFLAGS'].extend(
                ['-Wc,-pipe', '-Wc,--param,max-vartrack-size=200000000',
                 '-Wc,-mno-fma',
                 ...
                 '-C=alias', '-C=array', '-C=bits', '-C=dangling', '-C=do',
                 '-C=intovf', '-C=present', '-C=pointer', '-C=recursion'])
        ...
        if gpu == 'none':
            ...
            if '+cuda-gcc' in self.spec:
                ...
                cuda_host_compiler_stdcxx_libs = [
                    '-L{0}'.format(d) for d in
                    self._get_cuda_ccbin_link_paths(gcc, 'libstdc++')
                ]
        ...
        config_vars['LDFLAGS'].extend(['-L{0}'.format(d)
    ...
    def archive_files(self):
        ...
        # Archive makefiles:
        archive.extend([join_path(self.build_directory, f)
                        for f in ['Makefile', '*.mk']])
        ...
        with open(ccbin_wrapper, 'w') as f:
            f.writelines([
                ...
                'exec {0} {1} "$@"\n'.format(
                    compiler.cxx,
                    ' '.join(flag
                             for flag_type in ['cppflags', 'cxxflags']
